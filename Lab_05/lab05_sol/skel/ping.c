...
int ping(int sock, struct sockaddr_in *whereto, int count, char* from)
{
	int res;
	...
	for(int i = 0; i < count; i++) {
        ...
        printf("from %s: icmp_seq = %d", from, i);
        header.checksum = in_cksum((unsigned short *) (&header), sizeof(header), 0);
...
int main(int argc, char **argv)
{
	...
	ping(sock, &whereto, count, argv[2]);
