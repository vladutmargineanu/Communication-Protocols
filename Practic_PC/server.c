        ...
        for (i = 0; i <= fdmax; i++)
        {
            if (FD_ISSET(i, &tmp_fds))
            {
                if (i == sockfd)
                {
                    ...
                    clientslog[clients_num].username = buffer;


                    printf("Passwords: ");
                    memset(buffer, 0, BUFLEN);
                    fgets(buffer, BUFLEN - 1, stdin);
                    clientslog[clients_num].passw = buffer;

                    sprintf(notify_others, "[UPDATE] register: %d\n", newsockfd);
                    // sprintf(notify_others, "Username: ");
